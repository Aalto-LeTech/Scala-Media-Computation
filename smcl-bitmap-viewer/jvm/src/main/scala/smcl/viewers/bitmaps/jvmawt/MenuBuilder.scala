...
import smcl.infrastructure.StrEmpty
...
class MenuBuilder private(
  ...
  def menu(name: String, mnemonicOption: Option[Key.Value]): MenuBuilder = {
    ...
    if (_parents.lengthCompare(1) < 0 || !_parents.last.isInstanceOf[MenuBar])
  ...
  def get(): MenuBar = {
    ...
    if (_parents.lengthCompare(1) > 0)
      throw new IllegalStateException("Building of the current menu is not finished yet.")

    if (_parents.lengthCompare(1) < 0 || !_parents.head.isInstanceOf[MenuBar])
